...
class TestIntegration:
    ...
    def test_end_to_end_workflow(self):
        ...
        adc_data = generate_demo_data(size=8, noise_level=0.02)
        initial_mismatch = calculate_symmetry_mismatch(adc_data)
        
        # Calibrate
        calibrator = AdaptiveCalibrator(system_size=8)
        results = calibrator.calibrate(adc_data, target_tolerance=0.005)
        
        # Verify improvement
        if results['success']:
            assert results['final_mismatch'] < initial_mismatch
            assert results['improvement_ratio'] > 1.0
